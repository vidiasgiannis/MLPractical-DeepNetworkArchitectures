...
class ConvolutionalNetwork(nn.Module):
 ...
 #  My Proposed Solution for the two classes #
...
class BN_ConvolutionalProcessingBlock(nn.Module):
    ...
    def build_module(self):
        ...
        # Adding code here for batch normalization
        self.layer_dict['bn0'] = nn.BatchNorm2d(self.num_filters)
        out = self.layer_dict['conv_0'].forward(out)
        
        # Adding code here for normalizing the output
        out = self.layer_dict['bn0'](out)
        out = F.leaky_relu(out)
        ...
        # Adding code here for batch normalization
        self.layer_dict['bn1'] = nn.BatchNorm2d(self.num_filters)
        out = self.layer_dict['conv_1'].forward(out)

        # Adding code here for normalizing the output
        out = self.layer_dict['bn1'](out)
        out = F.leaky_relu(out)
        print(out.shape)
...
class RC_BN_ConvolutionalProcessingBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(RC_BN_ConvolutionalProcessingBlock, self).__init__()
    ...
    def forward(self, x):
        out = x
        # Adding code, identity definition
        identity = x
